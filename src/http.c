...
{
  self->status_line.version = realloc (self->status_line.version,
                                       strlen (version) + 1);
  self->status_line.reason_phrase = realloc (self->status_line.reason_phrase,
                                             strlen (reason_phrase) + 1);
...
void HTTP_receive (HTTP *self)
{
  ...
  while (1)
    {
      if (recv (self->pConn->fd, buffer, BUFFSIZE - 1, MSG_DONTWAIT) < 0
          && errno != EAGAIN)
        {
          free (receive);
          free (buffer);
...
//      1. parse the header, get the value of Content-Length;
      if ((header_size = parse_header (receive, self->pResponse)) > 0 &&
          (content_length = hashmap_get(&self->pResponse->headers, "Content-Length"))
          != NULL)
        {
          if ((strlen (receive) - header_size
               - strtol (content_length, NULL, 10)) >= 0)
            break; //FIXME: should not receive data more than content length says;
//               2. receive loop ends until Content-Length arrive;
        }
    }
//    3. copy body
  HTTPResponse_set_body (self->pResponse, &receive[header_size
                                                   + strlen (CRLF)]);
  free (buffer);
  free (receive);
}
